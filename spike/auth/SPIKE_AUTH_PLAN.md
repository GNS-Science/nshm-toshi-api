...
Self-contained in this `spike/auth/` directory. Uses the user's second AWS account with IAM Identity Center. Proves the end-to-end flow before committing to a production implementation.

### Deliverable 1: Cognito Setup Script
**`spike/auth/cognito_setup.py`** — boto3 script to create all Cognito resources:
...
Run once: `python spike/auth/cognito_setup.py --profile <your-aws-profile>`. Outputs config as JSON for use in subsequent steps.

### Deliverable 2: Scientist CLI Tool
**`spike/auth/toshi_auth.py`** — standalone Python script (uses `click` already in dev deps):
...
**`spike/auth/authorizer/handler.py`** — standalone Python Lambda function:
...
**`spike/auth/middleware.py`** — prototype `before_request` hook for `graphql_api/api.py`:
...
**`spike/auth/test_e2e.py`** — verifies all flows:
...
| File | Action |
|------|--------|
| `spike/auth/README.md` | Setup instructions + findings log |
| `spike/auth/cognito_setup.py` | boto3 Cognito provisioning script |
| `spike/auth/toshi_auth.py` | Scientist CLI tool |
| `spike/auth/authorizer/handler.py` | Lambda authorizer |
| `spike/auth/authorizer/requirements.txt` | `PyJWT`, `cryptography` (authorizer deps) |
| `spike/auth/middleware.py` | Flask middleware prototype |
| `spike/auth/test_e2e.py` | End-to-end validation |
