...
from nzshm_common.util import decompress_string, compress_string
...
class TestCompressRelations(unittest.TestCase):
    ...
    def test_create_file_relation_added_with_compression(self):
        ...
        self.assertEqual( json.loads(decompress_string(file.object_content['relations']))[-1], {'id': '100000', 'role': 'read'})
    ...
    def test_round_trip_file_relation_with_compression(self):
        ...
        self.assertEqual( file_result['data']['node']['relations']['edges'][0]['node']['thing']['id'], 'QXV0b21hdGlvblRhc2s6MTAwMDAw')
        self.assertEqual( file_result['data']['node']['relations']['edges'][0]['node']['thing']['__typename'], 'AutomationTask')
