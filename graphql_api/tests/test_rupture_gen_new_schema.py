...
from graphql_api.schema import root_schema, RuptureGenNewTask
...
class TestCreateRuptureGenNewTask(unittest.TestCase):
    """
    ...
    def test_create_minimum_fields_happy_case(self):
        ...
        print(executed)
        assert executed['data']['create_rupture_gen_new_task']\
                        ['task_result']['id'] == 'UnVwdHVyZUdlbk5ld1Rhc2s6MA=='
...
@mock.patch('graphql_api.data_s3.BaseS3Data.get_next_id', lambda self: 0)
@mock.patch('graphql_api.data_s3.BaseS3Data._write_object', lambda self, object_id, body: None)
class TestUpdateRuptureGenNewTask(unittest.TestCase):
    """
    All datastore (data_s3) methods are mocked.
    ...
    def test_update_with_metrics(self):
        qry = '''
            mutation {
                update_rupture_gen_new_task(input: {
                    task_id: "UnVwdHVyZUdlbk5ld1Rhc2s6MA=="
                    duration: 909,
                    rupture_count: 20
                })
                {
        ...
        executed = self.client.execute(qry)
        print(executed)
        result = executed['data']['update_rupture_gen_new_task']['task_result']
        assert result['id'] == 'UnVwdHVyZUdlbk5ld1Rhc2s6MA=='
