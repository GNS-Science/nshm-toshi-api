...
class Thing(graphene.Interface):
    ...
    def resolve_files(root, info, **args):
        ...
        if not root.files:
            ...
            db_metrics.put_duration(__name__, 'resolve_files[total_count]', dt.now(UTC) - t0)
            ...
            db_metrics.put_duration(__name__, 'resolve_files[optimised]', dt.now(UTC) - t0)

        else:
            # old form, files is list of strings
            res = [get_data_manager().file_relation.get_one(_id) for _id in root.files]
            db_metrics.put_duration(__name__, 'resolve_files[legacy]', dt.now(UTC) - t0)
    ...
    def resolve_parents(root, info, **args):
        ...
        logger.info('>> Thing.resolve_parents() ROOT: %s INFO: %s', root, info)
        ...
        if not root.parents:
            ...
            db_metrics.put_duration(__name__, 'resolve_parents[optimised]', dt.now(UTC) - t0)
        else:
            ...
            db_metrics.put_duration(__name__, 'resolve_parents[legacy]', dt.now(UTC) - t0)

        logger.info('>> Thing.resolve_parents() res: %s', res)
        return res

    def resolve_children(root, info, **args):
        t0 = dt.now(UTC)
        logger.info('>> Thing.resolve_children() ROOT: %s', root)
        logger.info('>> Thing.resolve_children() INFO: %s', info)
        ...
        if not root.children:
            ...
            db_metrics.put_duration(__name__, 'resolve_children[optimised]', dt.now(UTC) - t0)
        else:
            # old form, files is list of strings
            res = [get_data_manager().thing_relation.get_one(_id) for _id in root.children]
            db_metrics.put_duration(__name__, 'resolve_children[legacy]', dt.now(UTC) - t0)

        logger.info('>> Thing.resolve_children() res: %s', res)
        return res
