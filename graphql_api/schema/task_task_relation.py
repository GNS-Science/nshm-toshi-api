...
class TaskTaskRelation(graphene.ObjectType):
    ...
    def resolve_parent(root, info, *args, **kwargs):
        logger.info('TaskTaskRelation.resolve_parent() ROOT: %s ', root.parent_id)
        return get_data_manager().thing.get_one(root.parent_id)

    @staticmethod
    def resolve_child(root, info, *args, **kwargs):
        logger.info('TaskTaskRelation.resolve_child() ROOT: %s ', root.child_id)
...
class CreateTaskTaskRelation(graphene.Mutation):
    ...
    def mutate(self, info, **kwargs):
        t0 = dt.now(UTC)
        logger.debug("CreateTaskTaskRelation.mutate: %s", kwargs)
        ...
        logger.info("CreateTaskTaskRelation.mutate: thing_relation %s", thing_relation)

        db_metrics.put_duration(__name__, 'CreateTaskTaskRelation.mutate', dt.now(UTC) - t0)
