...
class CreateRuptureGenerationTask(graphene.Mutation):
    ...
    def mutate(cls, root, info, input):
        t0 = dt.now(UTC)
        log.info("CreateRuptureGenerationTaskmnutate %s", input)
        task_result = get_data_manager().thing.create('RuptureGenerationTask', **input)
        db_metrics.put_duration(__name__, 'CreateRuptureGenerationTask.mutate_and_get_payload', dt.now(UTC) - t0)
...
class UpdateRuptureGenerationTask(graphene.Mutation):
    ...
    def mutate(cls, root, info, input):
        t0 = dt.now(UTC)
        print("mutate: ", input)
        log.info("UpdateRuptureGenerationTask %s", input)
        thing_id = input.pop('task_id')
        log.info("UpdateRuptureGenerationTask thing_id %s", thing_id)
        task_result = get_data_manager().thing.update('RuptureGenerationTask', thing_id, **input)
        db_metrics.put_duration(__name__, 'UpdateRuptureGenerationTask.mutate_and_get_payload', dt.now(UTC) - t0)
