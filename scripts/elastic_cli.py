...
@click.option('-f', '--filter',
    type=click.Choice(['DAG', 'HAZ'], case_sensitive=False),
    help='list filter solutions by subtype.')
@click.option('-l', '--list-ids', is_flag=True, default=False, help="list the toshi_ids, with no aggregation (verbose option).")
...
def cli_OHS(filter, list_ids, verbose):
    ...
    if filter and filter=='DAG':
        if verbose: click.echo('GET disagggregation solutions')
    elif filter and filter=='HAZ':
        if verbose: click.echo('GET hazard solutions')
